#ifndef _IMGPIPE_SKEL_H
#define _IMGPIPE_SKEL_H
/// @file imgpipe.idl
///
#include "imgpipe.h"
#ifndef _QAIC_ENV_H
...
extern "C" {
#endif
static __inline int _skel_method(int (*_pfn)(unsigned char*, int, unsigned char*, int), uint32_t _sc, remote_arg* _pra) {
   remote_arg* _praEnd;
   ...
   _ASSERT(_nErr, (_pra + ((3 + 2) + (((0 + 0) + 0) + 0))) <= _praEnd);
   ...
   _COPY(_rout1Len, 0, _primIn, 12, 4);
   _ASSERT(_nErr, ((_praROut[1].buf.nLen / 1)) >= (size_t)(_rout1Len[0]));
   _rout1[0] = _praROut[1].buf.pv;
   _ASSERT(_nErr, (_rout0Len[0]) >= (size_t)(_in0Len[0]));
   _MEMMOVEIF(_rout0[0], (void*) _in0[0], (_in0Len[0] * 1));
   _ASSERT(_nErr, (_rout1Len[0]) >= (size_t)(_in1Len[0]));
   _MEMMOVEIF(_rout1[0], (void*) _in1[0], (_in1Len[0] * 1));
   _TRY(_nErr, _pfn(*_rout0, *_rout0Len, *_rout1, *_rout1Len));
   _CATCH(_nErr) {}
   return _nErr;
}
__QAIC_SKEL_EXPORT int __QAIC_SKEL(imgpipe_skel_invoke)(uint32_t _sc, remote_arg* _pra) __QAIC_SKEL_ATTRIBUTE {
   switch(REMOTE_SCALARS_METHOD(_sc))
   {
      case 0:
      return _skel_method((void*)__QAIC_IMPL(imgpipe_imageCPY), _sc, _pra);
   }
